...
-- Compact summary text settings
local SUMMARY_FONT = "ZoFontGameSmall"
local SUMMARY_COLOR = ZO_SELECTED_TEXT
-- Completed category (custom) constants and access
...
-- =====================================================
-- Pools (ESO-style status bars + rows for 2-column layout)
-- =====================================================
local function EnsureTooltipPools()
    if not AchievementTooltip then return end
    ...
    if not AchievementTooltip.nvk3utStatusBarPool then
        ...
        pool:SetCustomFactoryBehavior(function(control)
            ...
            local l = control:GetNamedChild("BGLeft");   if l then l:SetDrawLevel(2) end
            local r = control:GetNamedChild("BGRight");  if r then r:SetDrawLevel(2) end
            local m = control:GetNamedChild("BGMiddle"); if m then m:SetDrawLevel(2) end
        end)
        AchievementTooltip.nvk3utStatusBarPool = pool
    end

    -- Row pool (for 2 columns per row)
    if not AchievementTooltip.nvk3utRowPool then
        AchievementTooltip.nvk3utRowPool = ZO_ObjectPool:New(
            function()
    ...
    -- Release pools when tooltip clears
    if not AchievementTooltip._nvk3ut_onCleared then
        ZO_PreHookHandler(AchievementTooltip, "OnCleared", function()
            if AchievementTooltip.nvk3utStatusBarPool then AchievementTooltip.nvk3utStatusBarPool:ReleaseAllObjects() end
            if AchievementTooltip.nvk3utRowPool     then AchievementTooltip.nvk3utRowPool:ReleaseAllObjects()     end
            return false
        end)
...
local function ControlToNode(tree, ctrl)
    ...
    if ctrl.node then return ctrl.node end
    local c = ctrl
    for _ = 1, 4 do
...
local function ShowCategoryTooltip(data)
    EnsureTooltipPools()
    local ACH = GetACH()
    if not ACH or not ACH.GetCategoryIndicesFromData then return end
    local categoryIndex, subcategoryIndex = ACH:GetCategoryIndicesFromData(data)
    -- Guard: only handle valid base-game top-level categories
...
function ShowSummaryTooltip()
    -- Replicate Votan-style: build one multiline string and add it as a single tooltip line
    ...
    for i = 1, numTop do
        ...
        if name and name ~= "" then
            ...
            lines[#lines+1] = string.format("%s%s |cfafafa(%s/%s – %d%%)|r",
                iconTag, zo_strformat("<<1>>", name),
                ZO_CommaDelimitNumber(earned or 0), ZO_CommaDelimitNumber(total or 0), percent)
        end
    end
    ...
    if node and node.GetChildren and node:GetChildren() then
        for _, child in pairs(node:GetChildren()) do
            ...
            if d and d.subcategoryIndex then
                ...
                lines[#lines+1] = string.format("%s |cfafafa(%s Punkte)|r",
                    zo_strformat("<<1>>", name), ZO_CommaDelimitNumber(points or 0))
            end
        end
    else
        ...
        if Comp.GetSubcategoryList then
            ...
            if names and keys then
                for i=1,#keys do
                    ...
                    lines[#lines+1] = string.format("%s |cfafafa(%s Punkte)|r",
                        zo_strformat("<<1>>", names[i] or ""), ZO_CommaDelimitNumber(points or 0))
                end
            end
        end
    end
...
local function ShowCompletedSubTooltip(data)
    if not Comp or not data or not data.subcategoryIndex then return end
    local names, keys = Comp.GetSubcategoryList()
    local labelText = ""
    if names and keys then
        for i=1,#keys do
...
local function AttachHoverToLabel(tree, parentControl, label, MouseEnter, MouseExit)
    if not label or label._nvk3ut_tip then return end
    ZO_PreHookHandler(label, "OnMouseEnter", function() MouseEnter(parentControl) end)
    ZO_PreHookHandler(label, "OnMouseExit",  function() MouseExit(parentControl)  end)
...
local function HookHandlers(tree, control)
    ...
    local function MouseEnter(ctrl)
        ...
        if Nvk3UT and Nvk3UT.TryCustomCategoryTooltip and Nvk3UT.TryCustomCategoryTooltip(ctrl, data) then return end
        ...
        if data.isNvkCompleted or data.isNvkFavorites or data.isNvkRecent or data.isNvkTodo then isSummary = false end
        if U and U.d then U.d('[TT][Hover]', 'name=', tostring(name), ' C/F/R/T=', tostring(data.isNvkCompleted), '/', tostring(data.isNvkFavorites), '/', tostring(data.isNvkRecent), '/', tostring(data.isNvkTodo), ' isSummary=', tostring(isSummary)) end
...
if isSummary then
            if U and U.d then U.d('[TT] Summary') end
            ShowSummaryTooltip()
        else
            -- Robust detection of our custom "Abgeschlossen" category
            local isCompletedRoot =
                (data.isNvkCompleted == true) or
                (data.categoryIndex == NVK3_DONE) or
                (data.categoryId   == NVK3_DONE) or
                (data.id           == NVK3_DONE) or
                (data.categoryData and ((data.categoryData.id == NVK3_DONE) or (data.categoryData.index == NVK3_DONE))) or
                (name == "Abgeschlossen") -- fallback by label (DE)

            if isCompletedRoot then
                if data.subcategoryIndex then
                    if U and U.d then U.d('[TT] Completed SUB') end
                    ShowCompletedSubTooltip(data)
                else
                    if U and U.d then U.d('[TT] Completed ROOT') end
                ...
                if idx then
                    if U and U.d then U.d('[TT] Category') end
                    ShowCategoryTooltip(data)
                else
                    if U and U.d then U.d('[TT] Children') end
...
local function HookTemplates(tree)
    if not tree or not tree.templateInfo then return end
    local function setup(node, control, data, open) HookHandlers(tree, control) end
...
function T.HookNow()
    local ACH = GetACH()
    if not ACH or not ACH.categoryTree then return end
...
local function HookSceneAndEvents()
    ...
    EVENT_MANAGER:RegisterForEvent(T.name .. "_PA", EVENT_PLAYER_ACTIVATED, function()
        ...
        T.HookNow()
        zo_callLater(function() T.HookNow() end, 500)
        zo_callLater(function() T.HookNow() end, 2000)
    end)
    zo_callLater(function()
        if Nvk3UT and Nvk3UT.RebuildSelected and SecurePostHook then
            SecurePostHook(Nvk3UT, "RebuildSelected", function() T.HookNow() end)
        end
    end, 1000)
end
...
function T.Init()
    ...
    if sv then
        ...
        if sv.features.tooltips == nil then sv.features.tooltips = true end
...
---------------------------------------------
-- Nvk3UT: Custom Category Tooltips (Summary-Stil)
---------------------------------------------
Nvk3UT = Nvk3UT or {}

-- Defensive Helper: Node-Data aus dem Control ziehen, falls "data" nicht mitkommt
local function _nvkGetData(control, fallback)
    if fallback ~= nil then return fallback end
    if control and control.data then return control.data end
    if control and control.node and control.node.data then return control.node.data end
    local parent = control and control.GetParent and control:GetParent()
    if parent and parent.node and parent.node.data then return parent.node.data end
    return nil
end

-- Tooltip-Bausteine (Summary-Stil)
...
-- Sichere Modul-Resolver (keine Hard-Dependencies)
local function _nvkGetMod(modA, modB)
    local ok, mod = pcall(function()
        if _G[modA] then return _G[modA] end
        if _G[modB] then return _G[modB] end
        if Nvk3UT and Nvk3UT[modA] then return Nvk3UT[modA] end
        if Nvk3UT and Nvk3UT[modB] then return Nvk3UT[modB] end
        return nil
    end)
...
local function _nvkRenderFavorites(control)
    -- Summary-style: one multiline label, anchored like Summary
    ClearTooltip(AchievementTooltip)
    InitializeTooltip(AchievementTooltip, AchievementJournal, TOPRIGHT, 0, -104, TOPLEFT)
...
-- ------------- ABGESCHLOSSEN (Subkategorien mit Punkten) -------------
...
local function _nvkCompletedPointsForKey(Comp, key)
    ...
    if type(Comp.SummaryCountAndPointsForKey) == "function" then
        ...
        if ok then
            if tonumber(b) then return b end
            if tonumber(a) then return a end
        end
    end
    if type(Comp.PointsForSubcategory) == "function" then
        local ok, p = pcall(Comp.PointsForSubcategory, key)
        if ok and tonumber(p) then return p end
    end
    return 0
end
...
local function _nvkRenderCompleted(control)
    -- Use existing summary-style function to build single multiline label
    ClearTooltip(AchievementTooltip)
    InitializeTooltip(AchievementTooltip, AchievementJournal, TOPRIGHT, 0, -104, TOPLEFT)
    -- Try to get node via control (optional)
    local node = control and control.node
    if node == nil and control and control.GetParent then
        local p = control:GetParent()
        if p and p.node then node = p.node end
    end
    -- Delegate to existing builder (has fallback if node is nil)
...
-- ------------- TO-DO (Subkategorien: Punkte / Max TopCat) -------------
...
local function _nvkMaxPointsForTop(topId, Todo)
    if Todo and type(Todo.MaxPointsForTopCategory) == "function" then
        ...
        if ok and tonumber(m) then return m end
    end
    if type(GetAchievementCategoryInfo) == "function" and tonumber(topId) then
        ...
        if ok and tonumber(total) then return total end
    end
    return 0
end
...
local function _nvkRenderTodo(control)
    ...
    for i = 1, (keys and #keys or 0) do
        ...
        if maxTop and maxTop > 0 then
            lines[#lines+1] = string.format("%s |cfafafa(%s / %s)|r",
                zo_strformat("<<1>>", name), ZO_CommaDelimitNumber(pts or 0), ZO_CommaDelimitNumber(maxTop or 0))
        else
            lines[#lines+1] = string.format("%s |cfafafa(%s)|r",
                zo_strformat("<<1>>", name), ZO_CommaDelimitNumber(pts or 0))
        end
    end
    if #lines > 0 then
        local text = table.concat(lines, "\n")
