...
local function GetRowsOrWarn()
    local rows = GetRows()
    if not rows and not rowsWarningLogged then
        rowsWarningLogged = true
        local message = string.format("[%s] AchievementTrackerRows missing; skipping render", MODULE_NAME)
        if d then
            d(message)
        elseif print then
            print(message)
...
local function LayoutObjective(rows, achievement, objective)
    ...
    local control = rows:AcquireRow(NextRowKey(), "objective")
    rows:ApplyRow(control, "objective", {
        data = {
            achievementId = achievement.id,
            objective = objective,
        },
        font = state.fonts.objective,
        text = text,
        color = { GetAchievementTrackerColor("objectiveText") },
    })
...
local function LayoutAchievement(rows, achievement)
    local control = rows:AcquireRow(NextRowKey(), "achievement")
    ...
    local baseColor = { GetAchievementTrackerColor("entryTitle") }
    rows:ApplyRow(control, "achievement", {
        ...
        font = state.fonts.achievement,
        text = FormatDisplayString(achievement.name),
        baseColor = baseColor,
        updateIconSlot = UpdateAchievementIconSlot,
...
local function LayoutCategory(rows)
    ...
    local control = rows:AcquireRow(NextRowKey(), "category")
    ...
    rows:ApplyRow(control, "category", {
        ...
        expanded = expanded,
        baseColor = baseColor,
        selectCategoryToggleTexture = SelectCategoryToggleTexture,
        onMouseUp = function(ctrl, button, upInside)
            if not upInside or button ~= LEFT_MOUSE_BUTTON then
                return
            end
            local newExpanded = not IsCategoryExpanded()
        ...
        onMouseEnter = function(ctrl)
            ApplyMouseoverHighlight(ctrl)
            local isExpanded = ctrl.isExpanded
            if isExpanded == nil then
                isExpanded = IsCategoryExpanded()
            end
            UpdateCategoryToggle(ctrl, isExpanded)
        end,
        onMouseExit = function(ctrl)
            RestoreBaseColor(ctrl)
            local isExpanded = ctrl.isExpanded
