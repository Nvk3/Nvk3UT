...
QuestTrackerController.__index = QuestTrackerController

local MODULE_NAME = addonName .. "QuestTrackerController"
...
function QuestTrackerController.Init(parentControl, opts)
    ...
    if opts then
        QuestTrackerController.ApplyTheme(opts)
        QuestTrackerController.ApplySettings(opts)
    end
    ...
    if runtime and runtime.RegisterQuestTracker then
        runtime.RegisterQuestTracker({
            tracker = QuestTrackerController,
...
function QuestTrackerController.SetActive(active)
...
function QuestTrackerController.ApplySettings(settings)
...
function QuestTrackerController.OnTrackedQuestUpdate(trackingType, context)
    OnTrackedQuestUpdate(nil, trackingType, context)
end

function QuestTrackerController.OnPlayerActivated()
    if not state.isInitialized then
...
function QuestTrackerController.ApplyTheme(settings)
    if type(settings) ~= "table" then
        return
    end
...
function QuestTrackerController.ApplyHostVisibility(hidden, reason)
    ApplyHostVisibilityInternal(hidden, reason)
end

function QuestTrackerController.RequestRefresh()
    RequestRefresh()
end
