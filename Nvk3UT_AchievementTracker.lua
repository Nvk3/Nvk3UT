...
local function WarnMissingRowsHelper()
    ...
    DebugLog("AchievementTrackerRows missing; skipping render")
end
...
local function NextRowKey()
    state.nextRowKey = (state.nextRowKey or 0) + 1
    return state.nextRowKey
end
...
local function LayoutObjective(achievement, objective)
    ...
    rows:ApplyRow(control, "objective", {
        ...
        labelText = text,
        color = { r, g, b, a },
        fonts = { label = state.fonts.objective },
        callbacks = {
...
local function LayoutAchievement(achievement)
    ...
    rows:ApplyRow(control, "achievement", {
        ...
        color = { r, g, b, a },
        fonts = { label = state.fonts.achievement },
        callbacks = {
            OnAchievementMouseUp = function(ctrl, button, upInside)
...
local function LayoutCategory()
    ...
    rows:ApplyRow(control, "category", {
        ...
        callbacks = {
            OnCategoryMouseUp = function(ctrl, button, upInside)
